...
static int gf_get_gpio_dts_info(struct gf_device *gf_dev)
{
	...
	gf_dev->pins_irq = pinctrl_lookup_state(gf_dev->pinctrl_gpios, "eint_as_int");
	if (IS_ERR(gf_dev->pins_irq)) {
		ret = PTR_ERR(gf_dev->pins_irq);
		gf_debug(ERR_LOG, "%s can't find fingerprint pinctrl irq\n", __func__);
		return ret;
	...
	gf_dev->eint_in_low = pinctrl_lookup_state(gf_dev->pinctrl_gpios, "eint_in_low");
    	if (IS_ERR(gf_dev->eint_in_low)) {
    		ret = PTR_ERR(gf_dev->eint_in_low);
    		dev_err(&gf_dev->spi->dev, " Cannot find fp pinctrl eint_output_low!\n");
    		return ret;
    	}
    	gf_dev->eint_in_high= pinctrl_lookup_state(gf_dev->pinctrl_gpios, "eint_in_high");
    	if (IS_ERR(gf_dev->eint_in_high)) {
    		ret = PTR_ERR(gf_dev->eint_in_high);
    		dev_err(&gf_dev->spi->dev, " Cannot find fp pinctrl eint_in_high!\n");
    		return ret;
    	}
    
    	gf_dev->eint_in_float = pinctrl_lookup_state(gf_dev->pinctrl_gpios, "eint_in_float");
    	if (IS_ERR(gf_dev->eint_in_float)) {
    		ret = PTR_ERR(gf_dev->eint_in_float);
    		dev_err(&gf_dev->spi->dev, " Cannot find fp pinctrl eint_output_high!\n");
    		return ret;
    	}
#if 0
	gf_dev->pins_miso_spi = pinctrl_lookup_state(gf_dev->pinctrl_gpios, "miso_spi");
...
static void gf_reset_output(struct gf_device *pdev, int level)
{
	mutex_lock(&gf_set_gpio_mutex);
	//gf_print("%s: level = %d\n", __func__,level);

	if (level)
		pinctrl_select_state(pdev->pinctrl_gpios, pdev->pins_reset_high);
	else
		pinctrl_select_state(pdev->pinctrl_gpios, pdev->pins_reset_low);
	mutex_unlock(&gf_set_gpio_mutex);
}

static void gf_irq_pulldown(struct gf_device *pdev, int enable)
{
	mutex_lock(&gf_set_gpio_mutex);
	//gf_print("%s: enable = %d\n", __func__,enable);
	
	if (enable == 1) {
		pinctrl_select_state(pdev->pinctrl_gpios, pdev->eint_in_low);
	}
	else if(enable == 0) {
		pinctrl_select_state(pdev->pinctrl_gpios, pdev->eint_in_high);
	}
	else if(enable == -1) {
		pinctrl_select_state(pdev->pinctrl_gpios, pdev->eint_in_float);
	}
	mutex_unlock(&gf_set_gpio_mutex);
}

static int gf_power_ctl(struct gf_device* pdev, int on) 
{
    int rc = 0;
    ...
    if((on && !pdev->isPowerOn) || (!on && pdev->isPowerOn)) {
		if(on) 
		{
			...
			if (rc) { 
				gf_debug(DEBUG_LOG, "regulator_enable() failed!\n", __func__); 
				return rc;
			}
			udelay(400);
		...
		else 
		{
			...
			if (rc) {
				gf_debug(DEBUG_LOG,"regulator_disable() failed!\n",__func__);
				return rc;
			}
			msleep(50);
			pdev->isPowerOn = 0;
    	...
    	gf_debug(DEBUG_LOG,"Ignore %d to %d\n", on, pdev->isPowerOn);
    }
    return rc;
}

void gf_cleanup(struct gf_device * pdev) { }

int gf_power_init(struct gf_device* pdev) 
{
    int ret = 0;
    ...
    if(IS_ERR(pdev->avdd)) {
    	...
    	gf_debug(DEBUG_LOG, "Regulator get failed vdd ret=%d\n", ret);
    	return ret;
    }
	
    ret = regulator_set_voltage(pdev->avdd, GF_VDD_MIN_UV, GF_VDD_MAX_UV);
    if (ret) {
    	gf_debug(DEBUG_LOG, "regulator_set_voltage(%d) failed!\n", ret);
    	goto reg_vdd_put;
    }
	return 0;
...
static long gf_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	*/
	...
	{
	    gf_debug(INFO_LOG,"%s , GF_IOC_RELEASEVERSION",__func__);
	    if (copy_from_user(temp_buf, (int *)arg, sizeof(temp_buf))) {	
		gf_debug(INFO_LOG,"%s , FAIL TO COPY GF_IOC_RELEASEVERSION!",__func__);
		break;
	    }
	    full_fp_chip_info(temp_buf);
	}
	break;
	...
	case GF_IOC_INPUT_KEY_EVENT:
		...
		ftm_gfx_irq_key = 1;
		wake_up(&gf_poll_wq);//add by tinno
...
static unsigned int gf_poll(struct file *filp, struct poll_table_struct *wait)
{
	...
	poll_wait(filp, &gf_poll_wq, wait);
...
#endif
{
	...
	gf_debug(INFO_LOG, "get_fp_vendor =  %d\n", fp_vendor_tee);
	if (fp_vendor_tee != GOODIX_VENDOR) {
		gf_debug(INFO_LOG, "%s, Fingerprint vendor not goodix return!\n", __func__);
		status = -ENOMEM;
		goto err;		
	}
	//add by yinglong.tang
	...
	if (gf_power_init(gf_dev)) {
	    gf_debug(INFO_LOG, "gf_power_init err! \n", __func__);
	    goto err;
	}
...
static struct platform_driver fingerprint_dts_driver = {
	.probe = gf_probe,//fingerprint_dts_probe,
	.remove = gf_remove,//NULL,
	.shutdown = NULL,
	.driver = {
		   .name = GF_DEV_NAME,
...
static void __exit gf_exit(void)
{
	FUNC_ENTRY();
#if defined(USE_PLATFORM_BUS)
	platform_driver_unregister(&fingerprint_dts_driver);
#elif defined(USE_SPI_BUS)

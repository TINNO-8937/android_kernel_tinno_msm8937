...
*Abstract:	flash operation, read write etc.
*Author:      zuodong shao
*Date :        12 28,2015S
*Version:      1.0
*History :
...
#include "TINNO_V12_ICN86L8_20170920V4601"
};
...
int icn85xx_readVersion(void)
{
	...
	icn85xx_ts->ic_version = CurVersion;
	return CurVersion;
}
...
int icn85xx_goto_progmode(void)
{
	...
	flash_info("icn85xx_goto_progmode over, ret: %d\n", ret);
	if (retry == 0)
		return -EPERM;
...
unsigned short icn87xx_fw_Crc(unsigned short crc,
...
void ClearFlashState(void)/*void ClearFlashState()*/
{
	unsigned char ucTemp[4] = {0, 0, 0, 0};
...
#else/*QUAD_OUTPUT_ENABLE*/
void ClearFlashState(void)
{
}
#endif/*QUAD_OUTPUT_ENABLE*/
...
***********************************************************************************************/
int  icn85xx_prog_buffer(unsigned int flash_addr,
	unsigned int sram_addr, unsigned int copy_length, unsigned char program_type)
{
	unsigned char ucTemp[4] = {0, 0, 0, 0};
...
***********************************************************************************************/
void  icn85xx_read_flash(unsigned int sram_address,
	unsigned int flash_address, unsigned long copy_length, unsigned char i2c_wire_num)
{
	unsigned char ucTemp[4] = {0, 0, 0, 0};
...
int  icn87xx_fw_download(unsigned int offset,
...
int icn85xx_fw_update(void *fw)
{
	...
	icn85xx_ts->icn_is_update = 1;
	file_size = icn85xx_open_fw((char *)fw);
	if (file_size < 0) {
	...
	} else if ((ICN85XX_WITHOUT_FLASH_85 == boot_mode)
		|| (ICN85XX_WITHOUT_FLASH_86 == boot_mode)) {
		if (icn85xx_bootfrom_sram() == 0) {
...
int  icn87xx_fw_update(void *fw)
{
	...
	sram_len = (((temp_buf[9])<<8) + temp_buf[8]) + (((temp_buf[6])<<16)
		+ ((temp_buf[5])<<8) + temp_buf[4]) - (((temp_buf[2])<<16) + ((temp_buf[1])<<8) + temp_buf[0]);

...
s32 gt1x_read_version(struct gt1x_version_info * ver_info)
{
	...
	if (ver_info != NULL) {
		...
		sprintf(gtp_ver, "BOE-%s-GT%s-%x-%d",
			CONFIG_PRODUCT_NAME, product_id, patch_id, cfg_num);
		store_tp_info(gtp_ver);
	}
	return 0;
}
...
//extern interface for hall device to open/close smart cover mode
int smart_cover_switch(bool onoff)
{
	struct smart_cover_device *dev = gt1x_sc_dev;
	if (!dev) {
        return -ENODEV;
...
static struct kobj_attribute sc_attr = 
    __ATTR(state, S_IRUSR | S_IRGRP | S_IWUSR | S_IWGRP, smart_cover_show, smart_cover_store);
...
static ssize_t gt1x_force_fw_upgrade_store(struct device *dev,
				struct device_attribute *attr,
				const char *buf, size_t size)
{
	extern bool force_update_fw;
	force_update_fw = true;
	gt1x_update_firmware(NULL);
	force_update_fw = false;

	return size;
}

static DEVICE_ATTR(force_fw_upgrade, (S_IRUGO | S_IWUSR | S_IWGRP),
		NULL, gt1x_force_fw_upgrade_store);

static struct attribute *gt1x_attrs[] = {
	&dev_attr_force_fw_upgrade.attr,
	NULL
};

static const struct attribute_group gt1x_attr_grp = {
	.attrs = gt1x_attrs,
};

void gt1x_create_dev_attr(void)
{
	int ret;
	char upgrade_path[256] = { 0 };

	ret = sysfs_create_group(&gt1x_i2c_client->dev.kobj, &gt1x_attr_grp);
	sprintf(upgrade_path, "/sys/bus/i2c/devices/%d-%04x/force_fw_upgrade",
		gt1x_i2c_client->adapter->nr, gt1x_i2c_client->addr);

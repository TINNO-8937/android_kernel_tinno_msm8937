...
#define  TP_NOTIFY_WORK

#ifdef TP_NOTIFY_WORK
static struct work_struct fb_notify_work;
#endif
...
#ifdef CONFIG_CTP_POWER_GPIO_CTRL
		ret = gpio_direction_output(CTP_POWER_GPIO_NUM, 1);
		if (ret) {
			printk("set power_gpio on failed\n");
		}
#else
...
#ifdef CONFIG_CTP_POWER_GPIO_CTRL
		ret = gpio_direction_output(CTP_POWER_GPIO_NUM, 0);
		if (ret) {
			printk("set power_gpio off failed\n");
		}
#else
...
static s32 gt1x_request_io_port(void)
{
	...
	GTP_DEBUG_FUNC();
	ret = gpio_request(CTP_POWER_GPIO_NUM, "GTP_PWR_CTL");
...
static s8 gt1x_request_input_dev(void)
{
	...
	__set_bit(TINNO_GESTURE_KEY, input_dev->keybit);
...
#if   defined(CONFIG_FB)
#ifdef TP_NOTIFY_WORK
	INIT_WORK(&fb_notify_work, fb_notify_resume_work);
#endif

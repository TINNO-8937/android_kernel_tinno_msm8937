		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	if (rmi4_data->suspend && rmi4_data->enable_wakeup_gesture) {
		...
		gesture_type = rmi4_data->gesture_detection[0];
            ...
            else if(gesture_type==0x0B)
           {
        	   switch (rmi4_data->gesture_detection[2])
    		{
                    case 0x63:
                        gTGesture ='c';
                        input_report_key(syna_key_dev,KEY_SY_SENSOR, 1);
...
static int synaptics_rmi4_probe(struct platform_device *pdev)
{
	...
	if (! syna_key_dev) 
	{
		dev_err(&pdev->dev,"[syna]SY_key_dev: fail!\n");
	}
	...
	{
		dev_err(&pdev->dev,"[syna]SY_key_dev register : fail!\n");
	}else
	{
		dev_err(&pdev->dev,"[syna]SY_key_dev register : success!!\n");
	} 
#endif
	return retval;
...
*/
	...
	if (retval < 0) {
	    ...
	    //Begin<Release all points when open tgesture><><>;
	...
	input_sync(rmi4_data->input_dev);
    //End<Release all points when open tgesture><><>;
	return;
}

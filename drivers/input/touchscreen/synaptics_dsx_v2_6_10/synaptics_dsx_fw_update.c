...
#include <linux/input/synaptics_dsx_v2_6_10.h>
...
#define FW_IMAGE_NAME  "P7601_DJN_PR2511069-s3603_lock_30303130.img"
#define SYNA_IC_NAME "S3603R"
#define DO_STARTUP_FW_UPDATE
#else

#define FW_IHEX_NAME "synaptics/startup_fw_update.bin"
#define FW_IMAGE_NAME "V3941_DJN_PR2601427-s2503t_unlock_00000020.img"
#define SYNA_IC_NAME "S2503T"
#define DO_STARTUP_FW_UPDATE
...
static int save_tp_info(char *product_name, char *config_id)
{
	char buf[80];
	char *strptr;


	strptr = strnstr(fwu->image_name, "DJN", MAX_IMAGE_NAME_LEN);
	if (strptr) {

#if defined(CONFIG_PROJECT_P7601)
	sprintf(buf, "DIJING-%s-%s--V%d%d",CONFIG_PRODUCT_NAME,SYNA_IC_NAME,config_id[2]-0x30, config_id[3]-0x30);	
#else
	sprintf(buf, "DIJING-%s-%s--V%x",CONFIG_PRODUCT_NAME,SYNA_IC_NAME,config_id[31]);
#endif
	 
	}else{

	sprintf(buf, "UNKNOW-%s-%s--V%d%d",CONFIG_PRODUCT_NAME,SYNA_IC_NAME,config_id[2]-0x30, config_id[3]-0x30);

	}

        pr_notice("%s: geroge ---- buf = %s \n",__func__,buf);
	store_tp_info(buf);
...
pr_notice("%s: ---geroge------header->major_header_version-  = 0x%02x\n",__func__,header->major_header_version);
...
pr_notice("%s: ---geroge-  fwu->image_name = %s \n",__func__,fwu->image_name);
...
#if 0
pr_notice("%s: ---geroge-  image_fw_id = %d \n",__func__,image_fw_id);
pr_notice("%s: ---geroge--  device_fw_id = %d \n",__func__,device_fw_id);
...
static int fwu_start_reflash(void)
{
	...
	pr_notice("%s: Start 1111 of reflash process\n", __func__);
	...
	if (fwu->image == NULL) {
		...
		pr_notice("%s: Requesting firmware image %s\n",__func__, fwu->image_name);
...
pr_notice("%s: geroge flash_area = %d", __func__,flash_area);
	switch (flash_area) {
	case UI_FIRMWARE:
		do_rebuild = true;
...
static int fwu_start_recovery(void)
{
	...
	if (fwu->image == NULL) {
		...
		if (retval != 0) {
			dev_err(rmi4_data->pdev->dev.parent,
					"%s: Firmware ihex %s geroge not available\n",
					__func__, fwu->image_name);
			retval = -EINVAL;
			goto exit;
		}

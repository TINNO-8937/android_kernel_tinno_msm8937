...
static int fts_ctpm_check_need_upgrade(struct i2c_client *client)
{
	...
	//Add by Luoxingxing for c800.	
	else if (tinno_project_is("c800"))
	{
		FTS_INFO("[UPGRADE] %s:Select c800 project in FTS_RUN_IN_APP.", __func__);
	...
	{
		FTS_INFO("[UPGRADE] %s:Select c800 project in else.", __func__);

					...
					struct kgsl_memdesc *memdesc)
{
	...
	memdesc->gpuaddr = KGSL_IOMMU_SECURE_BASE(mmu);
		...
		struct kgsl_iommu_pt *pt)
{
	...
	if (mmu->secured && pagetable->name == KGSL_MMU_SECURE_PT) {
		pt->compat_va_start = KGSL_IOMMU_SECURE_BASE(mmu) + secure_global_size;
		...
		struct kgsl_iommu_pt *pt)
{
	...
	if (mmu->secured) {
		if (pagetable->name == KGSL_MMU_SECURE_PT) {
			...
			pt->va_end = KGSL_IOMMU_SECURE_BASE(mmu) + secure_global_size;
